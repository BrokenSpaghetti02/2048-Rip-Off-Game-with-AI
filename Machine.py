...
def play():
    ...
    def create_square(intensity):
        if intensity >= len(numbers):
            square = Square(0, randrange(5), choice(list(scores.keys())[:]))
        else:
            square = Square(0, randrange(5), choice(list(scores.keys())[:5 + intensity]))
        screen.fill((255,255,255), pygame.Rect(top_left_x, top_left_y-100,top_left_x+610,top_left_y))
        screen.fill(numbers[scores[square.value]], pygame.Rect(top_left_x + 100*square.column, top_left_y-100,100,100))
        screen.blit(scores[square.value], (top_left_x + 11 +square.column * 100, top_left_y + 7 - 100))
        pygame.display.flip()
        return square
    ...
    def check_and_merge(board, square):
        if board[square.row][square.column] != 0:
            ...
            board[square.row][square.column] = list(scores.keys())[list(scores.keys()).index(square.value) + count]
            square.value = list(scores.keys())[list(scores.keys()).index(square.value) + count]
            score += 2 * square.value * count
            score_change += 2 * square.value * count
            if count != 0:
                draw_board(board)
                draw_scoreBoard(score)
                #time.sleep(0.44)
    ...
    def collapse(board, square, column):
    ...
    def initiate_fall(square):
        ...
        while y != top_left_y+1+square.row*100:
            ...
            if square_fall_time/1000 >= square_fall_speed:
                ...
                screen.fill(numbers[scores[square.value]], pygame.Rect(top_left_x + 1 + 100*square.column, y,98,98))
                screen.blit(scores[square.value], (top_left_x + 11 + square.column*100, y + 7))
        ...
        for i in range(len(board)):
            for j in range(len(board[i])):
                if board[i][j] == 0:
                    ...
                    #pygame.draw.rect(screen, (10,10,10), (top_left_x + j*100, top_left_y+i*100,100,100),1)
                    screen.fill(numbers[scores[board[i][j]]], pygame.Rect(top_left_x + j*100, top_left_y+i*100,100,100))
                    screen.blit(scores[board[i][j]], (top_left_x + 11 + j*100, top_left_y + 7 + i*100))
    ...
    def check_check_and_merge(board, square):
        #print('called')
        ...
        if board[square.row][square.column] != 0:
            ...
            if square.row != 0 and board[square.row - 1][square.column] == square.value:
                ...
                board[square.row - 1][square.column] = 0
                fall = True
            #print(f'count for {square.value} on col {square.column} is {count}')
            board[square.row][square.column] = list(scores.keys())[list(scores.keys()).index(square.value) + count]
            square.value = list(scores.keys())[list(scores.keys()).index(square.value) + count]
            possible_score = 2 * square.value * count
            ...
            #print(f'fall {fall} again {again}')
            #print(f'new {board}')
            #print(f'row {square.row}')
            if fall:
                possible_score += check_check_and_merge(board, Square(square.row+1, square.column, square.value))
            elif again:
                possible_score += check_check_and_merge(board, square)
            #print(flattened_board != updated_flattened_board)
            if flattened_board != updated_flattened_board:
                for i in range(len(board)):
                    for j in range(len(board[i])):
                        possible_score += check_check_and_merge(board, Square(i,j,board[i][j]))
            #print(f'{square.column} score inside check is {possible_score}')
    ...
    scores = {2: two, 4: four, 8: eight, 16: sixteen, 32: thirty2, 64: sixty4, 128: hundred28,256:  two_hundred56,512: five_hundred12, 1024: thousand024,2048: two_thousand048}
    score = 0
    possible_score = 0
    score_change = 0
    board = [[0 for i in range(5)] for j in range(6)]
    #board = [[0 for i in range(5)] for j in range(2)] + [[1024,512,256,128,64]] +[[1024,512,2,128,64][::-1]] + [[1024,512,256,128,64]] + [[1024,512,2,128,64][::-1]]#test
    #print(board)
    clock = pygame.time.Clock()
    fall_time = 0
    fall_speed = 0.44
    intensity = 0
    events = [pygame.K_1, pygame.K_2, pygame.K_3, pygame.K_4, pygame.K_5]
    ...
    for i in range(1):
        while running:
            ...
            if score_change >= 5000:
                intensity += 1
                score_change = 0
            possible_scores = god_of_2048(board, current_square, score, intensity, depth = i)
            #print(possible_scores)
            machine_choice = events[possible_scores.index(max(possible_scores))]
            if machine_choice == pygame.K_1:
                if is_over(board, 0) == False:
                    if board[1][0] != 0:
                        next_square = create_square(intensity)
                        board[0][0] = current_square.value
                        screen.fill(numbers[scores[current_square.value]], pygame.Rect(top_left_x + 0*100, top_left_y+0*100,100,100))
                        screen.blit(scores[current_square.value], (top_left_x + 11 + 0*100, top_left_y + 7 + 0*100))
                        #print(board)
                        draw_board(board)
                        ...
                        next_square = create_square(intensity)
                        current_square = collapse(board, current_square, 0)
                        check_and_merge(board, current_square)
                        current_square = next_square
                        drop_sound.play()
            elif machine_choice == pygame.K_2:
                if not is_over(board, 1):
                    if board[1][1] != 0:
                        next_square = create_square(intensity)
                        board[0][1] = current_square.value
                        screen.fill(numbers[scores[current_square.value]], pygame.Rect(top_left_x + 1*100, top_left_y+0*100,100,100))
                        screen.blit(scores[current_square.value], (top_left_x + 11 + 1*100, top_left_y + 7 + 0*100))
                        #print(board)
                        draw_board(board)
                        ...
                        next_square = create_square(intensity)
                        current_square = collapse(board, current_square, 1)
                        check_and_merge(board, current_square)
                        current_square = next_square
                        drop_sound.play()
            elif machine_choice == pygame.K_3:
                if not is_over(board, 2):
                    if board[1][2] != 0:
                        next_square = create_square(intensity)
                        board[0][2] = current_square.value
                        screen.fill(numbers[scores[current_square.value]], pygame.Rect(top_left_x + 2*100, top_left_y+0*100,100,100))
                        screen.blit(scores[current_square.value], (top_left_x + 11 + 2*100, top_left_y + 7 + 0*100))
                        #print(board)
                        draw_board(board)
                        ...
                        next_square = create_square(intensity)
                        current_square = collapse(board, current_square, 2)
                        check_and_merge(board, current_square)
                        current_square = next_square
                        drop_sound.play()

            elif machine_choice == pygame.K_4:
                if not is_over(board, 3):
                    if board[1][3] != 0:
                        next_square = create_square(intensity)
                        board[0][3] = current_square.value
                        screen.fill(numbers[scores[current_square.value]], pygame.Rect(top_left_x + 3*100, top_left_y+0*100,100,100))
                        screen.blit(scores[current_square.value], (top_left_x + 11 + 3*100, top_left_y + 7 + 0*100))
                        #print(board)
                        draw_board(board)
                        ...
                        next_square = create_square(intensity)
                        current_square = collapse(board, current_square, 3)
                        check_and_merge(board, current_square)
                        current_square = next_square
                        drop_sound.play()
            elif machine_choice == pygame.K_5:
                if not is_over(board, 4):
                    if board[1][4] != 0:
                        next_square = create_square(intensity)
                        board[0][4] = current_square.value
                        screen.fill(numbers[scores[current_square.value]], pygame.Rect(top_left_x + 4*100, top_left_y+0*100,100,100))
                        screen.blit(scores[current_square.value], (top_left_x + 11 + 4*100, top_left_y + 7 + 0*100))
                        #print(board)
                        draw_board(board)
                        ...
                        next_square = create_square(intensity)
                        current_square = collapse(board, current_square, 4)
                        check_and_merge(board, current_square)
                        current_square = next_square
                        drop_sound.play()
            ...
            if is_over(board):
                ...
                plt.hist(final_score, density=True, bins=30)
                plt.ylabel('Scores')
                plt.xlabel('Depth')
...
def endscreen(score):
    while True:
        ...
        Score_RECT = ES_TEXT.get_rect(center=(700, 350))
